...
launch_sites = spacex_df.groupby("Launch Site").first().index
...
def get_scatter_chart(launch_site, payload):
    ...
    if launch_site == "ALL":
        ...
        fig = px.scatter(
            filtered_df[filtered_df['Launch Site'] == launch_site],
            x="Payload Mass (kg)",
            y="class",
            color="Booster Version Category",
            hover_data=["Launch Site"],
            title=f"Correlation Between Payload Mass and Success for Site {launch_site}",
        )
    return fig
